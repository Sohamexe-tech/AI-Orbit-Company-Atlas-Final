...
export async function generateMetadata({
  ...
  if (!result) {
    return { title: "Company not found | AI Orbit" };
  }

  return {
    title: `${result.company.name} | AI Orbit`,
    description: result.company.tagline,
  };
}
    ...
    <main>
          ...
          <div className="mt-10 grid gap-8 lg:grid-cols-[1fr_320px] lg:items-end">
            <div>
              <div className="flex flex-wrap items-center gap-3">
                <div className="flex size-16 items-center justify-center rounded-panel border border-border bg-black text-lg font-black">
                  {company.logoUrl ||
                    company.name.slice(0, 2).toUpperCase()}
                </div>

                <span className="rounded-full border border-border px-3 py-1 text-xs font-semibold text-fg-muted">
                  {company.industry}
                </span>
            ...
            <div className="rounded-panel border border-border bg-card p-5">
              <p className="eyebrow text-fg-subtle">Company snapshot</p>

              <div className="mt-4 space-y-4 text-sm">
                <div className="flex justify-between gap-4">
                  <span className="text-fg-subtle">Location</span>
                  <span className="text-right text-fg-soft">
                    {company.location}
                  </span>
                </div>

                <div className="flex justify-between gap-4">
                  <span className="text-fg-subtle">Founded</span>
                ...
                <div className="flex justify-between gap-4">
                  <span className="text-fg-subtle">Stage</span>
                  <span className="text-fg-soft">
      ...
      <Container className="py-12">
        <div className="grid gap-12 lg:grid-cols-[minmax(0,1fr)_320px]">
          <article>
            <p className="eyebrow text-accent-fg">About</p>

            <h2 className="mt-3 text-2xl font-bold">
              What {company.name} is building
            </h2>

            <p className="mt-5 max-w-3xl whitespace-pre-line text-base leading-8 text-fg-muted">
              {company.description}
            </p>
          </article>

          <aside className="rounded-panel border border-border bg-card p-5">
            <p className="eyebrow text-fg-subtle">Explore</p>
            ...
            <div className="mt-4 grid gap-2">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex h-10 items-center justify-between rounded-control border border-border px-3 text-sm font-semibold hover:bg-hover"
              >
                Website
